  ...
  return (
    <div id={'sagiri'}>
      <div className={'sagiriWrap'}>
        ...
        /
        <Link className={'sagiri'} href={`/characs/${params.name}/a_z`}>
          a_z
        </Link>
      </div>
      ...
      <div className={styles.erosNav}>
        ...
        <Link className={styles.navBlue} href={`/characs/${params.name}/a_z`}>
          a_z
        </Link>
      </div> 
      ...
      {searchParams.page <= 0 && data.length != 18 ? <></>:
        <Pagify p={`/characs/${params.name}/a_z?`} sagiri={data} page={searchParams.page} />}
    </div>
  )
